...
import {Affix, Button, Checkbox, Col, Divider, Row, notification} from 'antd';
  ...
  render() {
    ...
    const devtoolsEl = (
      <div style={{textAlign: 'right', opacity: 0.4}}>
        <h4>
          <Checkbox
            onClick={e => this.setState({displayDevTools: e.target.checked})}
          />
        </h4>
        {displayDevTools ? (
          <div>
            ...
            <Divider type="vertical" />
            <Button onClick={this.redraw.bind(this)}>
              <FormattedMessage
    ...
    if (annualYearsEls.length) {
        ...
        <Row key="summary">
          ...
          >
            <h3>
              <center>
                <FormattedMessage
                  id="MainCalculator.currentState"
      ...
      if (CONFIG.features.stocktakingNOW) {
        ...
        elements.push(
          <div key="stocktaking-now" style={{marginTop: '36px'}}>
            <TableStocktaking colors={colors} operation={stocktaking} />
          </div>,
        );
      }
    }
    ...
    // </div>);
    elements.push(
      <div hidden key={'download'}>
        <div style={{height: '6vw'}} />
        <Download operationQueue={operationQueue} />
      </div>,
    );
    let affix = false;
